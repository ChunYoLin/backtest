...
def _prepare_data(stock_pd, time_frame, output_length=1):
    ...
    result = []
    for idx in range(len(datavalue) - (time_frame+output_length)):
        result.append(datavalue[idx: idx + (time_frame+output_length)])

    result = np.array(result)
    number_train = round(0.9 * result.shape[0])

    x_train = result[:int(number_train), :-(output_length)]
    y_train = result[:int(number_train), -(output_length):][:,:,2]

    x_test = result[int(number_train):, :-(output_length)]
    y_test = result[int(number_train):, -(output_length):][:,:,2]
